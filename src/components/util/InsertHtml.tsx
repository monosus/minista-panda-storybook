...
function hxTriggerAttribute(options: HxTriggerOptions): string {
  ...
  if (options.event === 'intersect') {
    ...
    if (options.threshold !== undefined && options.threshold >= 0.0 && options.threshold <= 1.0) {
...
const InsertHtml = ({
  ...
  return (
    <>
      ...
      <include-file hx-get={processedHref} hx-trigger={hxTriggerAttribute(hxTrigger)}></include-file>
      {partName != null && <Comment text={`/ #${partName}`} />}
    </>
  );
};
