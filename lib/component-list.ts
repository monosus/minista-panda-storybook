...
const componentsDirectory = path.resolve(currentDirectory, '..', 'src', 'components');
...
function generateComponentExports(): void {
  const directories = fs.readdirSync(componentsDirectory).filter((item) => {
    ...
    return fs.statSync(itemPath).isDirectory() && fs.existsSync(path.join(itemPath, 'index.tsx'));
  });
...
function generateRecipeExports(): void {
  ...
  directories.forEach((dir) => {
    ...
    if (fs.existsSync(recipePath)) {
      recipeImports.push(`import { ${dir}Recipe } from './${dir}/${dir}.recipe';`);
      recipeKeys.push(`  ${dir}: ${dir}Recipe,`);
    }
  });

  const output = [...recipeImports, '', 'export const Recipes = {', ...recipeKeys, '};'].join('\n');
